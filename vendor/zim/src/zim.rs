...
impl Zim {
    ...
    pub fn new<P: AsRef<Path>>(p: P) -> Result<Zim> {
        ...
        let article_list = if header.title_ptr_pos == u64::MAX {
            // Title index pointer list is obsolete in newer namespace layouts.
            // Keep an empty list and rely on URL/path index iteration.
            Vec::new()
        } else {
...
fn parse_url_list(master_view: &Mmap, ptr_pos: u64, count: u32) -> Result<Vec<u64>> {
    ...
    let bytes = count.checked_mul(8).ok_or(Error::OutOfBounds)?;
    let end = start.checked_add(bytes).ok_or(Error::OutOfBounds)?;
...
fn parse_article_list(master_view: &Mmap, ptr_pos: u64, count: u32) -> Result<Vec<u32>> {
    let start = usize::try_from(ptr_pos)?;
    let count = usize::try_from(count)?;
    let bytes = count.checked_mul(4).ok_or(Error::OutOfBounds)?;
    let end = start.checked_add(bytes).ok_or(Error::OutOfBounds)?;
