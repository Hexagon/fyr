 ...
 * @returns {String} The basename of the URL path
 */
function getRemoteArchiveName (url) {
    try {
        var absolute = new URL(url, window.location.href);
        var pathname = absolute.pathname || '';
        var segments = pathname.split('/');
        var fileName = decodeURIComponent(segments[segments.length - 1] || 'remote.zim');
        return fileName || 'remote.zim';
    } catch (err) {
        // If URL parsing fails, use raw string fallback.
        var parts = (url || '').split('/');
        return parts[parts.length - 1] || 'remote.zim';
    }
}

/**
 * Reads archive size using HEAD or a tiny range fallback.
 * @param {String} url The archive URL
 * @returns {Promise<{ size: Number, acceptRanges: String|null }>} Archive metadata
 */
function readRemoteArchiveMetadata (url) {
    return fetch(url, {
        method: 'HEAD',
        credentials: 'same-origin'
    }).then(function (response) {
        if (!response.ok) {
            throw new Error('HTTP status ' + response.status + ' for HEAD ' + url);
        }
        ...
        // Fallback for servers that do not support HEAD but support Range.
        return fetch(url, {
            method: 'GET',
            credentials: 'same-origin',
            headers: {
            ...
            if (!(response.status >= 200 && response.status < 300)) {
                throw new Error('HTTP status ' + response.status + ' for probe GET ' + url);
            }
            var contentRange = response.headers.get('Content-Range');
            var match = contentRange && /bytes\s+\d+-\d+\/(\d+)/i.exec(contentRange);
            var size = match ? parseInt(match[1], 10) : parseInt(response.headers.get('Content-Length'), 10);
            if (!isFinite(size) || size < 0) {
                throw new Error('Unable to infer archive size for ' + url);
            }
            return {
                size: size,
                acceptRanges: response.headers.get('Accept-Ranges')
...
/**
 * Fetch a byte-range from a remote archive URL.
 * @param {String} url The archive URL
 * @param {Number} start Inclusive start byte
 * @param {Number} end Exclusive end byte
 * @param {Number} totalSize The total archive size
 * @returns {Promise<ArrayBuffer>} A promise for requested bytes
 */
function fetchRemoteArchiveRange (url, start, end, totalSize) {
    if (end <= start) return Promise.resolve(new ArrayBuffer(0));
    var rangeHeader = 'bytes=' + start + '-' + (end - 1);
    return fetch(url, {
        method: 'GET',
        credentials: 'same-origin',
        headers: {
        ...
        // Some servers may ignore Range and return full content for the complete-file request.
        if (response.status === 200 && start === 0 && end === totalSize) {
            return response.arrayBuffer();
        }
        throw new Error('Expected 206 for ' + rangeHeader + ' from ' + url + ', got ' + response.status);
    });
}

/**
 * Create a pseudo archive object that mimics File/Blob and serves byte ranges from URL.
 * @param {String} url The archive URL
 * @returns {Promise<Object>} A pseudo-file for the ZIM loader
 */
function readRemoteArchive (url) {
    return readRemoteArchiveMetadata(url).then(function (meta) {
        var totalSize = meta.size;
        var archiveName = getRemoteArchiveName(url);
        if ((meta.acceptRanges || '').toLowerCase() !== 'bytes') {
            console.warn('Remote archive server did not report Accept-Ranges: bytes for', url, meta.acceptRanges);
        }
        return {
            name: archiveName,
            size: totalSize,
            ...
            slice: function (begin, end) {
                var safeBegin = Math.max(0, begin | 0);
                var safeEnd = Math.min(totalSize, end == null ? totalSize : end | 0);
                return {
                    arrayBuffer: function () {
                        return fetchRemoteArchiveRange(url, safeBegin, safeEnd, totalSize);
                    }
                };
            }
        };
