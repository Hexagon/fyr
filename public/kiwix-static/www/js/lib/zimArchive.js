    ...
    var createZimfile = function (fileArray) {
        return zimfile.fromFileArray(fileArray).then(function (file) {
            ...
            return that.file.setListings([
                ...
                // Remote pseudo-file archives (URL-backed slice readers) are not structured-cloneable for worker transfer.
                var firstArchivePart = that.file._files[0];
                var isBlobLikeArchive = typeof Blob !== 'undefined' && firstArchivePart instanceof Blob;
                var isRemotePseudoZim = Boolean(firstArchivePart && (
                    firstArchivePart.sourceUrl || (!isBlobLikeArchive && typeof firstArchivePart.slice === 'function')
                ));
                if (that.file.fullTextIndex && (params.debugLibzimASM || !isSplitZim && typeof Atomics !== 'undefined' &&
                    ...
                    !/Android/.test(params.appType) && !(window.nw && that.file._files[0].readMode === 'electron')) && !isRemotePseudoZim ||
                    params.useLibzim && !isRemotePseudoZim) {
                    ...
                    } else if (isRemotePseudoZim) {
                        params.searchProvider += ': remote_stream';
                    } else if (isSplitZim) {
