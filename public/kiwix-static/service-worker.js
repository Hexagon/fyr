...
function fetchUrlFromZIM (urlObjectOrString, range, event/*, expectedHeaders*/) {
    return new Promise(function (resolve, reject) {
        ...
        // Wait for requestingFrameType to be resolved (it's async), then send messages to app clients.
        // In embedded deployments (like an iframe inside another app), there may be no top-level Kiwix client.
        // In that case, fall back to any available Kiwix client so requests do not hang indefinitely.
        getRequestingFrameType.then(function (requestingFrameType) {
            self.clients.matchAll().then(function (clientList) {
                ...
                var targetClients = topLevelClients.length ? topLevelClients : clientList;
